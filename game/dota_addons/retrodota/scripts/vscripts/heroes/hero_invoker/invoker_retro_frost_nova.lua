...
function invoker_retro_frost_nova_on_spell_start(keys)
	local frost_nova_particle_effect = ParticleManager:CreateParticle("particles/units/heroes/hero_lich/lich_frost_nova.vpcf", PATTACH_ABSORIGIN_FOLLOW, keys.target)
	...
	if quas_ability ~= nil then
		frost_nova_area_damage = keys.ability:GetLevelSpecialValueFor("primary_damage", quas_ability:GetLevel() - 1)
	end
	
	--Frost Nova's primary damage is dependent on the level of Exort.
	local exort_ability = keys.caster:FindAbilityByName("invoker_retro_exort")
	local frost_nova_primary_damage = 0
	if exort_ability ~= nil then
		frost_nova_primary_damage = keys.ability:GetLevelSpecialValueFor("primary_damage", exort_ability:GetLevel() - 1)
	end
	...
	for i, individual_unit in ipairs(nearby_enemy_units) do
		if individual_unit == keys.target then
			ApplyDamage({victim = individual_unit, attacker = keys.caster, damage = frost_nova_primary_damage, damage_type = DAMAGE_TYPE_MAGICAL,})
		else
